    ...
    $c->render( json =>
        {
          ...
          "scope"       => $c->url_for("/"), # maybe we can use /pwa here?!
          "display" => "standalone",
...
sub as_html( $base, $doc, %options ) {
    ...
    $base =~ s!/?$!/!; # make sure $base ends with a slash

    # Convert all relative links to links relative to $base/note/
    # this "only" affects links generated by Markdown::Perl, so a regex is
    # enough. We ignore manual / raw HTML links
    $html =~ s{<a\s+href="\K((?!/|https?:/)[^>"]*?)"}{$base$1"}gs;

    # Also do this for images and other attachments
...
% if( $type eq 'documents' ) {
    <div class="nav-item">
        <a href="#" data-bs-target="#sidebar" data-bs-toggle="collapse"
               id="btn-sidebar-labels"
               class="border rounded-3 p-1 text-decoration-none">Labels</a>
    </div>
...
<div id="actionbar" class="navbar bg-body-tertiary mt-auto fixed-bottom noprint">
    <div id="action-attach-image">
        ...
        >
            ...
            />
            <button type="submit" class="nojs">Upload</button>
            <img class="htmx-indicator" src="<%= url_for( "/icons/bars.svg" ) %>" alt="Uploading...">
        </form>
    </div>
